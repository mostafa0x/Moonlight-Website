...
export const SignOutButton: React.FC<{ isMobile?: boolean }> = ({ isMobile = false }) => {
  ...
  if (isMobile) {
    return (
      <button
        ...
        className="md:hidden mt-2 bg-rose-500 rounded-[10px] px-6 py-2 flex items-center justify-center gap-2.5 hover:bg-rose-600 transition-colors cursor-pointer"
      >
  ...
  return (
    <button
      onClick={handleSignOut}
      className="hidden md:flex w-40 h-14 bg-rose-500/40  rounded-[20px] items-center justify-center gap-2.5 hover:bg-rose-600 transition-shadow shadow-lg shadow-rose-500/20 cursor-pointer"
    >
      <img src={"/icons/sign-out.svg"} alt="sign out" />
