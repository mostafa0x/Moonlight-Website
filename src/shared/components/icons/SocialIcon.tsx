...
function SocialIcon({ icon }: { icon: IconType }) {
  ...
  return (
    <a
      ...
      className="relative flex items-center justify-center w-16 h-16 md:w-20.25 md:h-20.25 border border-white/20 rounded-full transition-all duration-300 hover:bg-white/10 hover:scale-110 active:scale-95 cursor-pointer select-none"
    >
      <img
        src={`/icons/${icon}.svg`}
        alt={`${icon} icon`}
        className="w-10 h-10 md:w-10.75 md:h-10.75 object-contain"
        loading="lazy"
      />
    </a>
  );
